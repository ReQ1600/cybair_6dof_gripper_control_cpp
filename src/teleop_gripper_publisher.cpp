...
constexpr char GRIPPER_TOPIC[] = "grip_val";
...
public:
    ...
    {
        teleop_subscription_ = this->create_subscription<std_msgs::msg::Float64>(GRIPPER_TOPIC, 1, std::bind(&PositionPublisher::parseAndSendToMoveit, this, std::placeholders::_1));
    }

    void MoveItSetup();
private:
    rclcpp::Subscription<std_msgs::msg::Float64>::SharedPtr teleop_subscription_;
    std::shared_ptr<moveit::planning_interface::MoveGroupInterface> move_group_interface_;
    const moveit::core::JointModelGroup* joint_model_group_;
    
    std::vector<double> joint_positions_;

    void parseAndSendToMoveit(const std_msgs::msg::Float64& delta_pos_msg);
};
...
void PositionPublisher::parseAndSendToMoveit(const std_msgs::msg::Float64& delta_pos_msg)
{
    auto& angle_change = delta_pos_msg.data;
    RCLCPP_INFO(LOGGER, "recieved: %f", angle_change);
    ...
    double& gripper_joint_val = joint_positions_[GRIPPER_JOINT_INDEX];
    gripper_joint_val += angle_change;
    
    //checking if position is within bounds
    if(!move_group_interface_->setJointValueTarget(joint_positions_)) RCLCPP_WARN(LOGGER, "Target gripper joint position: %f, out of bounds", gripper_joint_val);
    else
    {
        ...
        else RCLCPP_WARN(LOGGER, "planning move to gripper joint position: %f FAILED", gripper_joint_val);
    }
}
